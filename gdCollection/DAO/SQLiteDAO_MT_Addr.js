...
const filepath = path.resolve(__dirname, `../sqlites/${Config.sqliteMTAddr}`)
...
// 地址解析结果模型
const AddressModel = sequelize.define('T_Web_Dat_MT_Address', {
  ID: {
    primaryKey: true,
  ...
  street_num: DataTypes.TEXT,
  level: DataTypes.TEXT, // 地址类别
  CreateTime: DataTypes.DATEONLY
}, {
  timestamps: false,
  tableName: 'T_Web_Dat_MT_Address'
})

const SearchParamModel = sequelize.define('T_Web_MT_ExcuteParam', {
  ID: {
    primaryKey: '1',
    type: DataTypes.INTEGER
  },
  ...
  timestamps: false,
  tableName: 'T_Web_MT_ExcuteParam'
})
