...
function mars2DBMC(x, y) {
    let bd09XY = marsTobaidu({ x, y })
    return convertLL2MC({ x, y })
}
...
function getDistance(p1, p2) {　　 //角度转换为弧度
    ...
    var a = radLat1 - radLat2;//纬度弧度差值
    var b = getRad(p1.lng) - getRad(p2.lng);//经度弧度差值
    ...
    var distance = 2 * 6371393 * Math.asin(Math.sqrt(h));
 ...
 * @param {*} distance  分割距离 30米左右 非精确值
...
function coordinatesSplit(p1, p2, splitLength = 30) {
    ...
    for (let i = 0; i <= divisionsCount; i++) {
        let item = mars2DBMC((p1.lng + addX * i), (p1.lat + addY * i))
